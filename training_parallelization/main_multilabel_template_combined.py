"""
A template that creates python scripts for parallelized training and hyperparameter optimization using the combination of binary cross entropy
loss function and the ranking loss
"""
import pickle
import numpy as np
import torch
...
for counter, branch in enumerate([list_of_branches[int('branch_number')]]):
	...
	print(branch.go_term, output_size)
	...
	criterion_1 = nn.MultiLabelSoftMarginLoss()
	...
	for epoch in range(100):
		...
		for i, data in enumerate(train_loader, 0):
			...
			optimizer.zero_grad()
			outputs = net(inputs)
			loss = criterion_1(outputs, labels) + criterion_2(outputs, labels)
			loss.backward()
			optimizer.step()
