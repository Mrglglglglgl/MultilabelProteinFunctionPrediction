...
"""
a script that searches and saves the model architecture and weights for which the best f1
in the validation set was achieved using the combination of the ranking and the bce loss
"""
...
for branch_number, branch in enumerate(branch_list):
	...
	copyfile(pt_path, '/cluster/project1/FFPredRNN/MLDNN/' + str(branch.go_term) + 'combined_margin_' + margin + '.pt')
	file = open('/cluster/project1/FFPredRNN/MLDNN/' + str(branch.go_term) + 'trained_combined_margin_' + margin + '.pickle', 'wb')
