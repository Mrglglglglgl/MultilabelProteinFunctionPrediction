...
"""
a script that loads the best model developed using the ranking loss 
...
for counter, branch in enumerate(list_of_branches):
	mccs = []
	file = open('/cluster/project1/FFPredRNN/MLDNN/LTR_only_margin_' + margin + '/'+ str(branch.go_term) + 'trained_LTR_only_margin_' + margin + '.pickle', 'rb')
	...
	net.load_state_dict(torch.load('/cluster/project1/FFPredRNN/MLDNN/LTR_only_margin_' + margin + '/' + str(branch.go_term) + 'LTR_only_margin_' + margin + '.pt'))
	net.eval()
...
file = open('/cluster/project1/FFPredRNN/MLDNN/LTR_only_margin_' + margin + '/branches_metrics_LTR_only_margin_' + margin + '.pickle', 'wb')
