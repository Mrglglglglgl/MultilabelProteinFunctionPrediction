...
"""
a script that loads the best model developed using the combination of the BCE and the ranking loss 
to get the the metrics on the test set
"""
...
for counter, branch in enumerate(list_of_branches):
	mccs = []
	file = open('/cluster/project1/FFPredRNN/MLDNN/combined_margin_' + margin + '/'+ str(branch.go_term) + 'trained_combined_margin_' + margin + '.pickle', 'rb')
	...
	net.load_state_dict(torch.load('/cluster/project1/FFPredRNN/MLDNN/combined_margin_' + margin + '/' + str(branch.go_term) + 'combined_margin_' + margin + '.pt'))
	net.eval()
...
file = open('/cluster/project1/FFPredRNN/MLDNN/combined_margin_' + margin + '/branches_metrics_combined_margin_' + margin + '.pickle', 'wb')
