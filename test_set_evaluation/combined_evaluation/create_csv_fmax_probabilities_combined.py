...
"""
A script that creates the csv files containing the Fmax probabilities for each branch
when the combination of the BCE and the ranking loss function is used
...
file = open('branches_metrics_LTR_margin_' + margin + '.pickle', 'rb')
...
for index, branch in enumerate(branch_list):
	...
	preds_df.to_csv(str(branch.go_term) + '_preds_LTR_margin_' + margin + '.csv', sep=' ', float_format='%.2f', header=False)
	...
	labels_df.to_csv(str(branch.go_term) + '_labels_LTR_margin_' + margin + '.csv', sep=' ', float_format='%.2f', header=False)
